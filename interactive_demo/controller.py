...
class InteractiveController:
    ...
    def set_image(self, image):
        ...
        self.update_image_callback(self.image.shape)

    def set_mask(self, mask):
        if self.image.shape[:2] != mask.shape[:2]:
            print("Warning", "A segmentation mask must have the same sizes as the current image!")
    ...
    def finish_object(self):
        ...
        #cv2.imwrite('res.png', cv2.convertScaleAbs(self._result_mask*255)) #cv2.normalize(self.result_mask, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        self.object_count += 1
        self.reset_last_object()
    ...
    def reset_last_object(self, update_image=True):
        ...
        self.reset_init_mask()
        if update_image:
            self.update_image_callback(cv2.convertScaleAbs(self._result_mask*255))
